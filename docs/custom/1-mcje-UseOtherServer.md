...
+ 国内镜像站：[https://www.fastmirror.net/#/download/Vanilla](https://www.fastmirror.net/#/download/Vanilla?coreVersion=release)
...
将`BuildTools.jar`上传至实例文件的根目录，并在根目录下的`start.sh`文件内添加一行(不要删除start.sh自带的东西，echo那一行除外)：
...
+ Getbukkit(国外站，注意别点到服务器广告了)：[https://getbukkit.org/download/spigot](https://getbukkit.org/download/spigot)
...
```ini
${openjdk17} -Xms1024M -Xmx${maxmem}M -jar Spigot-1.20.2-3941 nogui
```
