...
bedrock:
  # 将侦听连接的IP地址。
  # 通常，如果要限制哪些IP可以连接到服务器，则应仅取消注释并更改此内容。
  #address: 0.0.0.0
  # 将侦听连接的端口
  port: 19132
  # 一些托管服务在每次启动服务器时更改您的Java端口并要求使用相同的端口用于Bedrock。
  # 此选项使Bedrock端口在每次启动服务器时与Java端口相同。
  # 此选项仅适用于插件版本。
  clone-remote-port: false
  # 将广播到Minecraft: Bedrock Edition客户端的MOTD。如果“passthrough-motd”设置为true，则此选项无关紧要
  # 如果其中任何一个为空，则相应的字符串将默认为“Geyser”
  motd1: "Geyser"
  motd2: "Another Geyser server."
  # 将发送到Minecraft: Bedrock Edition客户端的服务器名称。这在暂停菜单和设置菜单中可见。
  server-name: "Geyser"
  # 压缩网络流量到Bedrock客户端的程度。数字越高，使用的CPU越多，但
  # 使用的带宽越小。在-1以下或9以上没有任何效果。设置为-1以禁用。
  compression-level: 6
  # 广播到Bedrock客户端的端口，显示他们应该用来连接服务器的MOTD。
...
# 仅适用于在线模式身份验证类型。
# 存储应在登录后保存其Java Edition帐户的Bedrock玩家列表。
...
# 当第一次打开命令提示符时，Bedrock客户端可能会冻结。
# 禁用此功能将阻止发送命令建议并解决Bedrock客户端的冻结问题。
command-suggestions: true

# 以下三个选项启用“ping passthrough” - MOTD、玩家计数和/或协议名称从Java服务器检索。
# 将MOTD从远程服务器中继到Bedrock玩家。
passthrough-motd: true
# 将玩家计数和最大玩家从远程服务器中继到Bedrock玩家。
...
# 是否将玩家ping转发到服务器。启用此选项将允许Bedrock玩家具有更准确的
# ping，但也可能导致玩家更容易超时。
forward-player-ping: false

# 可连接的最大玩家数量。目前仅是视觉上的限制，实际上不会限制玩家数量。
max-players: 100
...
# 允许发送虚假冷却指示器。否则，Bedrock玩家不会看到冷却，因为他们仍在使用1.8战斗。
...
# 可以通过进入Bedrock设置，在可访问性选项卡下设置“文本背景不透明度”为0来禁用此功能
...
# 是否阻止Bedrock玩家执行他们的脚手架式桥接。
disable-bedrock-scaffolding: false

# 如果设置，当Bedrock玩家执行任何表情时，它将交换副手和主手物品，就像Java Edition键绑定一样
# 有三个选项可以设置：
# disabled - 默认/回退，不应用此解决方法
# no-emotes - 表情将不会发送到其他Bedrock客户端，副手将被交换。这实际上禁用所有表情的显示。
# emotes-and-offhand - 表情将发送到Bedrock客户端并交换副手
...
# 是否添加任何在Bedrock Edition中通常不存在的物品和方块。
...
# Bedrock阻止在地狱上方Y127处建造和显示方块。
# 此配置选项通过将地狱维度ID更改为末地ID来解决此问题。
# 这样做的主要缺点是整个地狱将具有相同的红色雾而不是每个生物群系都有不同的雾。
above-bedrock-nether-building: false

# 强制客户端加载所有资源包（如果有）。
# 如果设置为false，则允许用户连接到服务器，即使他们不想下载资源包。
force-resource-packs: true

# 允许解锁Xbox成就。
# 这将禁止所有Bedrock游戏内命令成功运行，否则Bedrock会认为您在作弊。
xbox-achievements-enabled: false

# 是否由服务器记录玩家IP地址。
log-player-ip-addresses: true

# 是否向控制台和操作员通知新版本的Geyser可用，该版本支持此Geyser版本不支持的Bedrock版本。
# 建议保持此选项启用，因为许多Bedrock平台会自动更新。
notify-on-new-bedrock-update: true

# 用于标记Bedrock玩家库存中不可用插槽的项目。这些示例包括在创意模式下的2x2配方网格，
...
# 是否尝试为Bedrock玩家禁用压缩。这应该是一个好处，因为在不处理Java数据包的情况下，没有必要压缩数据
# 当需要通过网络处理Java数据包时。
